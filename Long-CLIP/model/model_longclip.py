...
def is_huggingface_clip(state_dict: dict) -> bool:
    """HuggingFace CLIP 형식인지 확인"""
    return "vision_model.embeddings.patch_embedding.weight" in state_dict

def convert_hf_to_openai(hf_dict: dict) -> dict:
    """HuggingFace CLIP을 OpenAI CLIP 형식으로 변환"""
    import torch
    ...
    for key, value in hf_dict.items():
        ...
        openai_dict[new_key] = value
    
    # Attention weights 합치기
    openai_dict = _merge_qkv_weights(openai_dict, hf_dict)
    
    return openai_dict

def _merge_qkv_weights(openai_dict: dict, hf_dict: dict) -> dict:
    """분리된 Q, K, V를 in_proj로 합치기"""
    import torch
    
    # Vision layers
    vision_layers = len([k for k in hf_dict.keys() 
                        if k.startswith("vision_model.encoder.layers.") 
                        and k.endswith(".self_attn.q_proj.weight")])
    
    for i in range(vision_layers):
        prefix_hf = f"vision_model.encoder.layers.{i}.self_attn"
        prefix_openai = f"visual.transformer.resblocks.{i}.attn"
        
        openai_dict[f"{prefix_openai}.in_proj_weight"] = torch.cat([
            hf_dict[f"{prefix_hf}.q_proj.weight"],
            hf_dict[f"{prefix_hf}.k_proj.weight"],
            hf_dict[f"{prefix_hf}.v_proj.weight"]
        ], dim=0)
        
        openai_dict[f"{prefix_openai}.in_proj_bias"] = torch.cat([
            hf_dict[f"{prefix_hf}.q_proj.bias"],
            hf_dict[f"{prefix_hf}.k_proj.bias"],
            hf_dict[f"{prefix_hf}.v_proj.bias"]
        ], dim=0)
    
    # Text layers
    text_layers = len([k for k in hf_dict.keys() 
                      if k.startswith("text_model.encoder.layers.") 
                      and k.endswith(".self_attn.q_proj.weight")])
    
    for i in range(text_layers):
        prefix_hf = f"text_model.encoder.layers.{i}.self_attn"
        prefix_openai = f"transformer.resblocks.{i}.attn"
